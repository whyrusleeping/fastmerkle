...
cl_int run_benchmark(cl_context ctx, cl_device_id device, cl_program program, size_t iterations, bool test_use_host_ptr){
  ...
  printf("running my hacky benchmark\n");
  ...
  for(size_t i = 0; i < 10; i++){
    ...
    for(size_t j = 0; j < iterations; j++){
      uint8_t* data = calloc(500, SHA256_DIGEST_SIZE * sizeof(uint8_t));
      bool mutated = false;

      int64_t start = time_now();

      err = compute_tree(ctx, device, program, 0, data, &mutated, false);
      if(err != CL_SUCCESS){
        free(data);
...
cl_int compute_tree(cl_context ctx, cl_device_id device, cl_program program, size_t leaves, uint8_t* data, bool* mutated_ptr, bool use_host_ptr){
  ...
  cl_mem data_buf;
  if(use_host_ptr){
    data_buf = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, SHA256_DIGEST_SIZE * threads * sizeof(uint8_t), data, &err);
    if(err != CL_SUCCESS) return err;
  } else {
    data_buf = clCreateBuffer(ctx, CL_MEM_READ_WRITE, SHA256_DIGEST_SIZE * threads * sizeof(uint8_t), NULL, &err);
    if(err != CL_SUCCESS) return err;

    err = clEnqueueWriteBuffer(queue, data_buf, CL_TRUE, 0, SHA256_DIGEST_SIZE * threads * sizeof(uint8_t), data, 0, NULL, NULL);
    if(err != CL_SUCCESS) return err;
  }
  ...
  cl_kernel kernel = clCreateKernel(program, "dohashes", &err);
  if(err != CL_SUCCESS) return err;
